...
|void addCustomTool(String icon, String name, String category, Object callback)|5.0 |Adds a custom tool with SVG icon, name and a given action specified by callback to the toolbox of the Notes application under the given category. Only tool categories with a context menu are allowed. If category is left empty, tool will be added under "More" category.
|===
