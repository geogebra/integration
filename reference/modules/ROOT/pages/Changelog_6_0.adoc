...
== 6.0.930.1 (3rd September 2026 build)
* Improve copy/paste to Excel
* Algebra View: fixes for output row 
* Keyboard navigation improvements
* bugfixes for new Settings

== 6.0.929.3 (19th August 2026 build)
* 3D: crop extra long text so it displays OK
* CAS: commands with random outputs now update on F9 etc
* CAS: fix for `Circle(A,B)`
* Access spreadsheet cells in applets with `<Tab>`
* SVG Export: fix for images
* bugfixes for new Settings

== 6.0.927.1 (6th July 2026 build)
* New Settings dialogs
* New Text Tool dialog
* New "visual" editor for integrals
* `sqrt(-2)` now works in Input Boxes linked to complex numbers
* Classic 5: fix selection in the Algebra View
* fix for selecting sub-expressions with <Shift>arrows or mouse
* Localization added for screen reader
* CAS: added `Midpoint/Center()` command
* Accessibility improvements for Probability Calculator

== 6.0.925.3 (15th May 2026 build)
* Classic 6: now uses fast spreadsheet code
* Dynamic color implemented in new spreadsheet
* Classic 5: fix bug where scripting text lost on focus change
* CAS: fix for eg `Numeric(gamma(1+i))`
* bugfix: onClick scripts fail in shadow DOM
* better method used for `HALF_UP` rounding 
* make sure spreadsheet XML is minimal
* bugfix: `ClosestPoint()` wrong for hyperbola
* new option "Simplify Coefficients" to omit zero coefficient terms in a function
* "Size" option for Pie Charts
* Classic 5: make sure Settings headings don't get jumbled
* Smoother export to .STL for `Curve()`
* Speedup for applets where bounds change
* bugfix: make sure files load in the correct subapp on Android
* bugfix: "Dots" grid setting doesn't repsect "Positive direction only"
* fix online loading for Electron apps
* `enableProbability` option added to App Parameters
* numbers over `9007199254740991` are now printed in standard form

== 6.0.920.0 (10th March 2026 build)
* Classic 5: fix opacity slider

== 6.0.919.1 (5th March 2026 build)
* Classic 5: fix `\fbox{}`

== 6.0.918.0 (25th February 2026 build)
* Compile with GWT v2.13.0
* 3D points get a "halo" when selected with `<Tab>`
* Classic: fix for Algebra View commands in CAS
* Box Plots can be dragged vertically
* CAS: fix for eg `Numeric(4.5,1)`
* improve intersections for rational functions
* fix for correlation in Regression dialog

== 6.0.916.2 (17th February 2026 build)
* fix for `((5 sqrt(2))/(2 sqrt(10)))`
* fix Relation Tool in eg Spanish
* Suite: add more calculations into Spreadsheet menu
* Classic 5: fix menu spacing

== 6.0.915.1 (26th January 2026 build)
* Classic 5: fix opacity slider
* CAS: Improve inequality auto-simplifying
* fix slider drawing & XML logic when min ≥ max
* fix List Tool with TableView points

== 6.0.913.0 (13th January 2026 build)
* Fix detection of dependent variable in `SolveODE()`

== 6.0.911.3 (8th January 2026 build)
* Classic 5 installer updated to `v5.4.911.3` / Java 25 (Intel macOS no longer supported)
* Use better font for `∞` in the Algebra View
* fix for clipped buttons in image export dialog
* make sure Construction Order preserved in Algebra View
* Box Plot added to Chart menu in spreadsheet

== 6.0.909.9 (12th December 2025 build)
* fix `f'` for conic
* fix redefining `f(z)`
* macOS: better font for Algebra View
* "Zoom to Fit" button added in Graphics View

== 6.0.907.0 (18th November 2025 build)
* speed up saving/loading spreadsheet numbers
* Better screen reader output for matrix input boxes

== 6.0.906.2 (11th November 2025 build)
* speed up parsing / file loading
* make sure `f'(x)` returns factored answer as appropriate
* new API for custom tools in Notes

== 6.0.905.1 (31st October 2025 build)
* make sure `$point`, `$vec$` don't leak onto the clipboard when copying a command
* make sure LaTeX line breaks don't occur in numbers
* fix for display of `(-4<x≤3) && (-1<x<3)`
* Classic 5: EPS export removed
* "type to rename" option removed

== 6.0.904.2 (22nd October 2025 build)
* Classic 5: JOGL updated to v2.6
* `Sequence(Sequence(...` shown as matrix in Algebra View
* Bugfix: make sure `Point(list of points)` reloaded correctly
* New command `SetLineOpacity( <Object>, <Number> )`
* Improved screenreader output for `TableText()`
* fix for `Point(polyhedron)`

== 6.0.903.0 (7th October 2025 build)
* fix for eg `(8 sqrt(2))/sqrt(3)`

== 6.0.902.0 (30th September 2025 build)
* *Can be installed as PWA offline app in browsers*
* Electron updated to `v38.2`
...
* LaTeX: support for text wrapping in tables eg `\begin{array}{lp{200}}a&\text{very long content could break}\\c&d\\\end{array}`
...
* Probability Calculator: round inputs to discrete distributions correctly for non-integers
